  ...
  import { 
    ...
    LogOut, Menu, X, Activity, UserCheck, Pencil, Check, XCircle, Loader2 
  } from "lucide-react";
  ...
  export default function AdminPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch ke API Dashboard yang Anda berikan
        // Pastikan mengirim Authorization Header sesuai kebutuhan API
        ...
        // Set data sesuai struktur JSON yang Anda kirim
    ...
    const handleSaveUser = async () => {
      ...
      try {
        ...
        // Panggil API PUT (Pastikan file route.ts ada di /api/admin/users)
        ...
        if (res.ok) {
          // Update local state tanpa refresh halaman
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 scroll-smooth">
            ...
            {isLoading ? (
              ...
              <>
                ...
                {/* USERS VIEW (UPDATED WITH EDIT FUNCTIONALITY) */}
                {activeMenu === "users" && (
                  <div className="animate-fade-in">
                    <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
                      <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold">Semua Data User</h3>
                        <button className="px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-bold flex items-center gap-2 shadow-lg shadow-cyan-500/20">
                          <UserCheck size={16} /> Total: {users.length} User
                        </button>
                      </div>
                    <div className="overflow-x-auto">
                          ...
                          <tbody className="divide-y divide-white/5 text-sm">
                            {users.map((u) => (
                              <tr key={u.id} className="hover:bg-white/5 transition-colors group">
                                ...
                                <td className="p-4">
                                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${u.role === 'admin' ? 'bg-purple-500/20 text-purple-300' : 'bg-blue-500/20 text-blue-300'}`}>{u.role}</span>
                                </td>
                                <td className="p-4 text-slate-400">{new Date(u.createdAt).toLocaleDateString('id-ID')}</td>
                                <td className="p-4">
                                  <button 
                                    onClick={() => openEditModal(u)}
                                    className="p-2 rounded-lg bg-slate-800 hover:bg-cyan-600 hover:text-white transition-colors"
                                  >
                                    <Pencil size={16} />
                                  </button>
                                </td>
                ...
                {activeMenu === "doctors" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                    <div className="border border-dashed border-white/10 rounded-2xl p-10 text-center text-slate-500">
                      <p>Fitur Data Dokter (Sesuaikan dengan Users View)</p>
                    </div>
                  </div>
                )}
